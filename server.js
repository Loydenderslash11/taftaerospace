...
function removedDataFileFor(stationsFile) {
  return path.join(path.dirname(stationsFile), "removed-stations.json");
}

async function readRemovedStationIds() {
  const dataFile = await getDataFile();
  const removedFile = removedDataFileFor(dataFile);

  try {
    const raw = await fsp.readFile(removedFile, "utf8");
    const parsed = JSON.parse(raw);
...
async function writeRemovedStationIds(ids) {
  const dataFile = await getDataFile();
  const removedFile = removedDataFileFor(dataFile);
  const uniqueIds = [...new Set(ids.filter(Boolean))];
  await fsp.writeFile(removedFile, `${JSON.stringify(uniqueIds, null, 2)}\n`);
}
...
async function handleApi(request, response) {
  if (request.method === "GET") {
    try {
      sendJson(response, 200, {
        stations: await readStations(),
        removedIds: await readRemovedStationIds()
      });
    } catch {
  ...
  if (request.method === "DELETE") {
    try {
      ...
      const removedIds = await readRemovedStationIds();

      if (nextStations.length === stations.length) {
        await writeRemovedStationIds([...removedIds, id]);
