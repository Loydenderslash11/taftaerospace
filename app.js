...
function updateSyncStatus() {
  syncStatus.textContent = sharedStoreAvailable
    ? "Shared mode active: new stations will appear for everyone visiting this hosted site."
    : "Private mode: open through server.js to share new stations with every visitor.";
  clearAdded.disabled = sharedStoreAvailable;
  clearAdded.textContent = sharedStoreAvailable ? "Shared Save On" : "Clear Private Stations";
}
...
function renderVideoCard(video, index, query) {
  ...
  const canRemove = Boolean(video.custom);
...
async function removeStation(id) {
  ...
  if (sharedStoreAvailable && station.shared) {
